...
export default function TabelaTurmas({ turmas, selecionarTurmaParaEdicao, confirmarPovoamento }) {
  return (
      ...
      <thead>
        <tr>
          {["SÉRIE", "ANO LETIVO", "LETRA", "SALA", "PROFESSOR", "AÇÕES"].map((t, i) => (
            <th
              key={i}
              className="px-6 py-4 text-center text-xs font-semibold text-gray-300 uppercase bg-gray-800/80"
            >
              {t}
            </th>
          ))}
          ...
          turmas.map((turma, index) => (
            <tr key={index} className="hover:bg-gray-700/40">
              ...
              <td className="px-6 py-4 text-sm text-center text-gray-300 space-x-2">
                <button
                  onClick={() => selecionarTurmaParaEdicao(turma)}
