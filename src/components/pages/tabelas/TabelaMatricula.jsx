...
import { apagarMatricula, buscarMatriculas } from '../../../redux/matriculaReducer';
...
export default function TabelaMatricula(props) {
  ...
  /*useEffect(() => {
    dispatch(buscarMatriculas());
  }, [dispatch]);*/

  function excluirMatricula(matricula) {
    if (window.confirm(`Deseja realmente excluir ${matricula.nome}?`)) {
      dispatch(apagarMatricula(matricula));
    }
  }

  function alterarMatricula(matricula) {
    props.setModoEdicao(true);
    props.setExibirTabela(false);
    props.setMatricula(matricula)
  }

  const matriculasFiltradas = listaDeMatriculas.filter(matricula =>
    matricula.nome.toLowerCase().includes(pesquisa.toLowerCase())
  );
    ...
    <div className=" py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="sm:flex sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Cadastro de Matriculas</h1>
          ...
          >
            <UserPlus className="h-4 w-4 mr-2" /> Nova Matricula
          </button>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {matriculasFiltradas.map((matricula) => (
                  <tr key={matricula.id}>
                    <td className="px-6 py-4 whitespace-nowrap">{matricula.nome}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{matricula.cpf}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{matricula.rg}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{new Date(matricula.dataNascimento).toLocaleDateString()}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{matricula.sexo == 'M' || matricula.sexo=='m' ? 'Masculino' : 'Feminino'}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{matricula.estadoCivil}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <button onClick={() => alterarMatricula(matricula)} className="text-indigo-600 hover:text-indigo-900 mr-4">
                        <Pencil className="h-5 w-5" />
                      </button>
                      <button onClick={() => excluirMatricula(matricula)} className="text-red-600 hover:text-red-900">
