...
export default function TabelaSalas(props) {
  ...
  function excluirSala(sala) {
    if (window.confirm(`Deseja realmente excluir a sala: ${sala.descricao}?`)) {
      dispatch(apagarSalas(sala));
    }
  }

  function alterarSala(sala) {
    props.setSalaEdicao(true);
    props.setExibirTabela(false);
    props.setSala(sala);
  }

  const salasFiltradas = listaSalas.filter(sala =>
    sala.ncarteiras.toString().includes(pesquisa)
  );
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {salasFiltradas.map((sala) => (
                    <tr key={sala.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">{sala.descricao}</td>
