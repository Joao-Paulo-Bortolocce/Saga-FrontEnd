...
export default function FormularioPovoarTurma({ turmaSelecionada, cancelar }) {
    ...
    async function gravarMatriculas(e) {
        ...
        const promises = selecionados.map(ra => {
            ...
            return fetch("http://localhost:8080/matricula", {
                ...
                if (!res.ok) throw new Error("Erro ao salvar matrícula");
                return res.json();
            });
        });
    ...
    return (
        <form onSubmit={gravarMatriculas} className="bg-white text-black p-6 rounded-xl space-y-4 max-w-xl mx-auto border border-gray-300 shadow-xl">
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto border p-3 rounded bg-gray-100">
                {matriculas.length === 0 && (
                    <p className="text-gray-500">Nenhum aluno disponível para esta turma.</p>
                )}
            ...
            <div className="flex justify-between">
                <button type="submit" className="bg-green-600 px-4 py-2 rounded hover:bg-green-700 text-white">
